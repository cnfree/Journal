...
# 第九章
  * OLAP 是CPU密集型操作，OLTP是IO密集型操作
  * 一条SQL只能在一个CPU中工作，并不支持多CPU的处理
  * 缓冲池的增大，TPS会线性增长，直到所有文件操作都在内存中进行
  * InnoDB缓冲池的命中率不应小于99%
  * 缓冲池命中率
  ![缓冲池命中率]
  * 可以通过 SHOW GLOBAL STATUS LIKE 'innodb%read%'\G 查看各种状态来计算命中率
  * 即使缓冲池大小大于数据库文件大小，也还有IO操作，因为需要将脏页写入到磁盘，还要讲日志记录到重做日志
  * 15000 RPM 磁盘寻道时间能达到3ms，顺序访问速度远远高于随机访问
  * 闪存读取速度远远高于写入速度
  * 固态硬盘的访问延时一般小于0.1ms
  * 可以通过增大innodb_io_capacity的值才充分利用固态硬盘带来的高IOPS特性
  * RAID组合分为：
    * RAID 0: 按磁盘个数分段，速度快，没有冗余功能
    * RAID 1: 两个以上的N个磁盘互作镜像，可靠性高，利用率低
    * RAID 5: 至少需要三个硬盘，不进行备份，利用剩下的数据和奇偶校验进行数据恢复，RAID 0 和 RAID 1 的折中方案
    * RAID 01: 先分区再镜像，比RAID10更快
    * RAID 10: 先镜像再分区，安全性更高，同一组即使全坏，也能工作，至少四块硬盘
    * RAID 50: 至少6块硬盘
  * 数据库最好使用 RAID10，同时兼顾 RAID 0 和 RAID 1 的特性  
